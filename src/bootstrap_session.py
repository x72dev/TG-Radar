...
async def main() -> None:
    ...
    for target in [
        config.admin_session.with_suffix(".session"),
        config.admin_worker_session.with_suffix(".session"),
        config.core_session.with_suffix(".session"),
    ]:
    ...
    print(f"- {config.admin_worker_session.with_suffix('.session')}")
