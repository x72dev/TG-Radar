...
CREATE TABLE IF NOT EXISTS plugin_state (
    ...
    fuse_threshold INTEGER NOT NULL DEFAULT 5,
...
LOG_PRESENTATION = {
    ...
    "PLUGIN": ("normal", "🟡", "插件管理", "插件状态变更"),
    "PLUGIN_FUSE": ("critical", "🔴", "插件熔断", "插件因连续错误被自动停用"),
}
...
def _humanize_log(action: str, detail: str, fallback: str) -> tuple[str, str]:
    ...
    if action in {"ADMIN", "CORE"}:
        if "started" in detail or "已启动" in detail:
            return ("服务已启动", detail)
        if "stopping" in detail or "正在停止" in detail:
            return ("服务正在停止", detail)
    if action in {"SYNC", "AUTO_SYNC"}:
...
class RadarDB:
    ...
    # FIX BUG-01: proper connection lifecycle management
    @contextmanager
    def _read(self) -> Iterator[sqlite3.Connection]:
        """Context manager for read-only operations that properly closes connections."""
        conn = self._connect()
        try:
            yield conn
        finally:
    ...
    # FIX BUG-02: upsert_folder now properly updates enabled/alert on conflict
    def upsert_folder(
        self,
        folder_name: str,
        folder_id: int | None,
        ...
        def _apply(c: sqlite3.Connection) -> None:
            ...
            if existing is None:
                ...
                c.execute(
                    "UPDATE folder_rules SET folder_id=?, enabled=?, alert_channel_id=?, updated_at=? WHERE folder_name=?",
                    (folder_id, effective_enabled, effective_alert, now, folder_name),
                )
    ...
    # PERF-01: batch counts in one query
    def count_cache_all_folders(self) -> dict[str, int]:
        with self._read() as conn:
            rows = conn.execute("SELECT folder_name, COUNT(*) as cnt FROM system_cache GROUP BY folder_name").fetchall()
            return {row["folder_name"]: int(row["cnt"]) for row in rows}

    def count_rules_all_folders(self) -> dict[str, int]:
        with self._read() as conn:
            rows = conn.execute("SELECT folder_name, COUNT(*) as cnt FROM keyword_rules WHERE enabled=1 GROUP BY folder_name").fetchall()
            return {row["folder_name"]: int(row["cnt"]) for row in rows}
    ...
    def get_plugin_state(self, name: str) -> dict[str, Any] | None:
        with self._read() as conn:
    ...
    def list_plugin_states(self) -> list[dict[str, Any]]:
        with self._read() as conn:
            rows = conn.execute("SELECT * FROM plugin_state ORDER BY kind, name").fetchall()
            return [dict(r) for r in rows]

    def upsert_plugin_state(self, name: str, kind: str, source: str, enabled: bool = True) -> None:
        now = self._now()
        with self.tx() as conn:
            conn.execute(
                "INSERT INTO plugin_state(name, kind, source, enabled, installed_at, updated_at) VALUES (?, ?, ?, ?, ?, ?) "
                "ON CONFLICT(name) DO UPDATE SET kind=excluded.kind, source=excluded.source, updated_at=excluded.updated_at",
                (name, kind, source, int(enabled), now, now),
            )

    def set_plugin_enabled(self, name: str, enabled: bool) -> bool:
        with self.tx() as conn:
            cur = conn.execute(
                "UPDATE plugin_state SET enabled=?, fuse_count=0, updated_at=? WHERE name=?",
                (int(enabled), self._now(), name),
            )
            return cur.rowcount > 0

    def increment_plugin_fuse(self, name: str) -> int:
        """Increment fuse counter and return new count."""
        with self.tx() as conn:
            conn.execute(
                "UPDATE plugin_state SET fuse_count=fuse_count+1, updated_at=? WHERE name=?",
                (self._now(), name),
            )
            row = conn.execute("SELECT fuse_count, fuse_threshold FROM plugin_state WHERE name=?", (name,)).fetchone()
            if row is None:
                return 0
            return int(row["fuse_count"])

    def reset_plugin_fuse(self, name: str) -> None:
        with self.tx() as conn:
            conn.execute(
                "UPDATE plugin_state SET fuse_count=0, updated_at=? WHERE name=?",
                (self._now(), name),
            )

    def get_plugin_config(self, name: str) -> dict[str, Any]:
        with self._read() as conn:
            row = conn.execute("SELECT config_json FROM plugin_state WHERE name=?", (name,)).fetchone()
            if row is None:
                return {}
    ...
    def set_plugin_config(self, name: str, config: dict[str, Any]) -> None:
        with self.tx() as conn:
            conn.execute(
                "UPDATE plugin_state SET config_json=?, updated_at=? WHERE name=?",
                (json.dumps(config, ensure_ascii=False), self._now(), name),
            )

    def delete_plugin_state(self, name: str) -> bool:
        with self.tx() as conn:
            cur = conn.execute("DELETE FROM plugin_state WHERE name=?", (name,))
            return cur.rowcount > 0
