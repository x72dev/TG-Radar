    ...
    async def _execute_sync(self, job: AdminJob, automatic: bool) -> JobResult:
        ...
        if route_report.queued:
            self.app.command_bus.submit(
                "route_apply",
                priority=55,
                dedupe_key="route_apply",
                origin="system",
                visible=False,
                delay_seconds=self.config.route_apply_delay_seconds,
            )
        if sync_report.has_changes or route_report.created or route_report.queued:
            ...
            self.app.command_bus.submit(
                "reload_core",
                payload={"reason": "sync_changed", "detail": f"changed={sync_report.has_changes}; queued={sum(route_report.queued.values())}; created={len(route_report.created)}"},
                priority=40,
