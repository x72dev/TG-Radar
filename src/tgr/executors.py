...
class AdminExecutors:
    ...
    async def execute(self, job: AdminJob) -> JobResult:
        dispatch = {
            ...
            "reload_state": lambda: self._reload_state(job),
            "reload_core": lambda: self._reload_state(job),  # 兼容旧名
        }
        handler = dispatch.get(job.kind)
        if handler:
            return await handler()
        return JobResult(status="done", summary=f"未实现: {job.kind}", log_action="COMMAND", log_level="ERROR")
    ...
    async def _sync(self, job: AdminJob, automatic: bool) -> JobResult:
        ...
        if has_changes:
            ...
            # 直接重载运行时状态
            await self.app.reload_runtime_state()
    ...
    async def _update(self, job: AdminJob) -> JobResult:
        ...
        changed = sorted(set([n for n, t in after.items() if before.get(n) != t] + [n for n in after if n not in before]))
        return JobResult(status="done", summary="更新完成", detail="\n".join(outputs), payload={"changed_plugins": changed}, log_action="UPDATE")
    ...
    async def _reload_state(self, job: AdminJob) -> JobResult:
        """直接重载运行时状态，同进程内调用。"""
        reason = str(job.payload.get("reason", "runtime_change"))
        await self.app.reload_runtime_state()
        return JobResult(status="done", summary="状态已重载", detail=reason, log_action="STATE_RELOAD")
